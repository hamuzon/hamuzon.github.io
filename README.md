# ホームページリンク / Homepage Links

## メインサイト / Main Site

- [hamusata.f5.si](https://hamusata.f5.si/)  
  メインのホームページです。  
  Main homepage.

- [GitHub リポジトリ / GitHub Repository](https://github.com/hamuzon/hamusata.f5.si)  
  このサイトの GitHub リポジトリです。  
  GitHub repository of this site.

---

## GitHub 版ホームページ / GitHub Pages Version

- [hamuzon.github.io](https://hamuzon.github.io)  
  Hamuzon の GitHub Pages 版ホームページです。  
  The GitHub Pages version of Hamuzon's homepage.

---

## ポートフォリオサイト / Portfolio Site

- [home.hamusata.f5.si](https://home.hamusata.f5.si)  
  メインサイト hamusata.f5.si のポートフォリオ版です。  
  Portfolio site of the main homepage.

---

## ツールまとめサイト / Tools Summary Site

- [www.link-s.f5.si](https://www.link-s.f5.si)  
  いろいろなツールをまとめたサイトです。  
  A site summarizing various tools.

---

## 参考用 / Reference / Separate Repository

- [hamuzon-jp.f5.si](https://hamuzon-jp.f5.si)  
  参考用に別リポジトリで作られたサイトです。  
  Created in a separate repository for reference.

- [GitHub リポジトリ / GitHub Repository](https://github.com/hamuzon/hamuzon-jp.f5.si)  
  上記サイトの GitHub リポジトリです。  
  GitHub repository of the above site.

---

リンク先の作品もぜひご覧ください。  
Please feel free to explore the works linked from these sites.
