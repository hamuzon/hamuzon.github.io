(() => {
  ...
  class Snowflake {
    ...
    reset() {
      ...
      // 10%確率で特別オブジェクト
      if (Math.random() < 0.10) {
        const types = ['bigflake','snowman','star','heart','gift','tree','bell','snowflakeFancy'];
    ...
    draw(ctx) {
      ...
      switch(this.shape){
        ...
        case 'star':
          ...
          ctx.beginPath();
          for(let i=0;i<5;i++){
            ctx.lineTo(this.radius*Math.cos((18+i*72)*Math.PI/180),this.radius*Math.sin((18+i*72)*Math.PI/180));
            ctx.lineTo(this.radius/2*Math.cos((54+i*72)*Math.PI/180),this.radius/2*Math.sin((54+i*72)*Math.PI/180));
          }
          ctx.closePath(); ctx.fill();
          break;
  ...
  const snowCount = 120;
  const flakes = [];
  for(let i=0;i<snowCount;i++) flakes.push(new Snowflake());
